...
# 設計・実装・完了処理フェーズ（fs-bugfix-phase2-impl）

バグ修正ワークフローの Phase 2。差分設計、タスク計画、差分実装、設計書反映を実行する。

## The Iron Laws

- **担当外に踏み込まない**: バグ報告のヒアリング、原因分析、修正方針の策定をしてはならない。Phase 1（バグ報告・原因分析・修正方針）の範囲に踏み込んではならない
- **既存設計書の直接変更禁止**: 既存設計書を直接変更してはならない。更新が必要な場合は fix-design.md の「更新が必要な設計資料」セクションに記載し、完了処理で doc-sync 経由で反映する
- **大規模設計時の分割対応**: 修正設計の規模が大きい場合（目安: 全体 300〜500 行超、または個別項目で 50 行超が複数）、サブエージェントは fix-design.md を「メイン（索引）+ 分割ファイル群」構成で作成する。後続 Step（タスク計画 / doc-sync / design-sync）では、メインの索引から分割ファイルを発見し、すべて入力として読み込むこと。索引のみ読んで本文を読み忘れることは厳禁
...
呼び出しパラメータ:
- skill_name: `fs-bugfix-phase2-impl`
...
1. **progress-resume-check (aide-powers skill)** を activate して実行する
   ...
   - 戻り値による分岐:
     - `RESUME_FROM N` → N が本フェーズ（2）なら該当 Step から再開
     - `START_FRESH` → 異常。バグ報告・原因分析・修正方針が未完了。前フェーズスキルに差し戻す
...
5. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `前処理`, step_title: `前処理`, artifact_dir: `{bugfix_dir}`）
...
1. fix-plan.md + bug-analysis.md を読み込む
2. 影響を受ける設計領域を特定する:
   - ユーザー要件 / システム要件 / GUI設計 / オブジェクト設計 / ユビキタス言語 / インフラIF設計 / プログラム構成
3. 変更規模による分岐:
   - **局所的修正（1〜2ファイル）** → Step 2 へ（bugfix-designer のみで作成）
   - **広範囲修正（複数領域）** → 影響を受ける各設計系共通スキルを **`mode: delta`** で activate して実行。各スキルが `{bugfix_dir}/delta-{領域名}.md` に差分を出力 → Step 2 で統合
4. 設計系共通スキル対応表:
...
5. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step1`, step_title: `設計系共通スキル呼び出し判定`, artifact_dir: `{bugfix_dir}`）

#### Step 2: 修正設計の作成（サブエージェント委譲）

本スキルディレクトリの `bugfix-designer-prompt.md` を Read で読み込み、プレースホルダーを置換してサブエージェントをディスパッチする。

**タスク:** 修正方針に基づき、具体的なコード変更の差分設計を before→after 形式で作成する
...
| 設計系共通スキルの差分設計結果 | Step 1 の結果（広範囲修正時のみ） |

**Output:** `{bugfix_dir}/fix-design.md`（規模が大きい場合は加えて `{bugfix_dir}/fix-design-{name}.md` を分割ファイルとして作成）
...
**Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step2`, step_title: `修正設計の作成`, artifact_dir: `{bugfix_dir}`）

#### Step 3: 修正設計のユーザー承認

**処理:**

1. `{bugfix_dir}/fix-design.md` の内容を確認する
   - メインファイルの「修正対象の差分設計」「新規追加の設計」が分割ファイル索引（リンク一覧）になっている場合、各分割ファイル `{bugfix_dir}/fix-design-{name}.md` も読み込み、ユーザー提示時にメイン+全分割ファイルをセットで提示する
   - 分割されていない場合はメインファイルのみ提示する
2. ユーザーの判断による分岐:
   - **承認** → Step 4 へ進む
   - **修正要求** → サブエージェントを fix モードで再ディスパッチし、修正後 Step 3 を再実行する
3. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step3`, step_title: `修正設計のユーザー承認`, artifact_dir: `{bugfix_dir}`）
...
3. 戻り値による分岐:
   - **APPROVED** → Step 6 へ
   - **REJECTED** → Step 5 へ
4. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step4`, step_title: `修正設計のQAレビュー`, artifact_dir: `{bugfix_dir}`）
...
本スキルディレクトリの `bugfix-designer-prompt.md` を Read で読み込み、fix モードでサブエージェントをディスパッチする。

**タスク:** QAレビューからの指摘に基づいて fix-design.md を修正する
...
| fix_design_path | bugfix_dir から構築（メインファイル。分割ファイルが存在する場合はサブエージェントが索引から特定して該当の分割ファイルを Edit する） |
...
**Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step5`, step_title: `QA REJECTED 修正ループ`, artifact_dir: `{bugfix_dir}`）

### タスク計画区画

#### Step 6: 差分タスクリストの作成（サブエージェント委譲）

> **リグレッションテスト必須。** fix-plan.md の「リグレッションテスト方針」が空でない限り、リグレッションテストタスクは必ず存在する。

本スキルディレクトリの `bugfix-task-planner-prompt.md` を Read で読み込み、プレースホルダーを置換してサブエージェントをディスパッチする。

**タスク:** 修正設計と修正方針をもとに、依存関係を考慮した実装タスクリストと工程チェック表を作成する
...
| fix_design_path | bugfix_dir から構築（メインファイル。サブエージェントは索引判定をして分割ファイル群も全て Read で読み込む） |
| fix_plan_path | bugfix_dir から構築 |
| bug_analysis_path | bugfix_dir から構築 |

**Output:** `{bugfix_dir}/delta-task-list.md`, `{bugfix_dir}/impl-process-checklist.md`

**ステータス分岐:**
- `DONE` → Step後処理を実行し、Step 7 へ
- `DONE_WITH_CONCERNS` → 懸念事項の内容をユーザーに報告し、対応方針を確認した上で Step 7 へ
- `NEEDS_CONTEXT` → 追加情報を提供して再ディスパッチ
- `BLOCKED` → ユーザーに報告し対応方針を確認

**Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step6`, step_title: `差分タスクリストの作成`, artifact_dir: `{bugfix_dir}`）

#### Step 7: タスクリストのユーザー承認

**処理:**

1. `{bugfix_dir}/delta-task-list.md` の内容をユーザーに提示する
2. ユーザーの判断による分岐:
   - **承認** → Step 8 へ進む
   - **修正要求** → サブエージェントを再ディスパッチして修正後 Step 7 を再実行する
3. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step7`, step_title: `タスクリストのユーザー承認`, artifact_dir: `{bugfix_dir}`）
...
1. `{bugfix_dir}/impl-process-checklist.md` の存在を確認する
2. 存在する → Step 9 へ
3. 存在しない → ワークフロー中断。ユーザーに「工程チェック表が存在しません。Step 6（タスク計画）に戻って作成してください」と報告し、実装作業を一切開始しない
4. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step8`, step_title: `工程チェック表存在確認 HARD-GATE`, artifact_dir: `{bugfix_dir}`）

#### Step 9: 実行計画策定

**処理:**

1. `{bugfix_dir}/delta-task-list.md` を読み込む
...
7. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step9`, step_title: `実行計画策定`, artifact_dir: `{bugfix_dir}`）

#### Step 10: タスク実装ループ（multi-stage-code-review 経由）
...
`delta-task-list.md` のサブタスク1つにつき、実装サイクル（micro-impl-agent による implement/write_test/run_test と、multi-stage-code-review 経由の design-review-agent / code-review-agent によるレビュー）を1回回す。1呼び出しで複数サブタスクを束ねてはならない。
...
3. 以下のサイクルを実行する（各エージェントに上記ペイロードテンプレートの全項目を渡す）:
   1. `micro-impl-agent (aide-powers agent)`（mode: implement）→ 実装コードを書く
   2. `multi-stage-code-review (aide-powers skill)` を Stage 1（実装コードレビュー）で activate して実行する。内部で design-review-agent + code-review-agent のレビュー、FAIL 時の micro-impl-agent(mode: fix)による修正→再レビューが全PASSまで回る
   3. `micro-impl-agent (aide-powers agent)`（mode: write_test）→ テストコードを書く
   4. `multi-stage-code-review (aide-powers skill)` を Stage 2（テストコードレビュー）で activate して実行する
   5. `micro-impl-agent (aide-powers agent)`（mode: run_test）→ テストを実行する
   - multi-stage-code-review は Stage 1（実装レビュー）/ Stage 2（テストレビュー）を担い、初期実装（implement）・テスト作成（write_test）・テスト実行（run_test）は micro-impl-agent が担う
   - multi-stage-code-review 内部で「依頼内容チェック」が実行され、複数サブタスク統合等の違反がある場合 BLOCKED で返される
4. 非プログラム成果物の場合は簡略サイクル: 実装 → 設計準拠レビューのみ → 完了
5. レビュー結果受領後の判断フロー実行（Step 11）
6. タスク完了 → 次のタスクへ
7. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step10`, step_title: `タスク実装ループ`, artifact_dir: `{bugfix_dir}`）

注: step-history-writer は各タスク完了時ではなく、全タスクのレベル完了時または Step 10 全体完了時に1回呼び出す。

#### Step 11: レビュー結果受領後の判断フロー
...
2. `PASS_WITH_DEVIATION`（合理的乖離検出）の場合:
   ...
   - fix-design.md を修正、必要に応じて delta-task-list.md を再構築（fix-design.md が分割構成になっている場合は、該当の分割ファイル側を修正する）
   - 並列実行中なら他タスクを一時停止
   - 設計同期完了後、修正後のタスクから再開
3. 注: FAIL は multi-stage-code-review 内部で修正→再レビューが完了しているため、オーケストレータに FAIL が返ることはない
4. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step11`, step_title: `レビュー結果受領後の判断フロー`, artifact_dir: `{bugfix_dir}`）

注: Step 11 は Step 10 の各タスクサイクル内で実行されるため、step-history-writer は Step 10 完了時にまとめて記録する。
...
3. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step12`, step_title: `リグレッションテスト`, artifact_dir: `{bugfix_dir}`）

#### Step 13: ユーザー動作検証依頼

**処理:**

1. バグが修正された状態になったことをユーザーに報告する
2. 動作検証を依頼する。伝える内容:
   - 修正したバグの概要
   - 再現手順での確認方法
   - 確認してほしいポイント（バグが再現しないこと）
   - 影響がある既存機能
3. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step13`, step_title: `ユーザー動作検証依頼`, artifact_dir: `{bugfix_dir}`）

### 完了処理区画

#### Step 14: 設計書反映（サブエージェント委譲）

本スキルディレクトリの `bugfix-doc-syncer-prompt.md` を Read で読み込み、プレースホルダーを置換してサブエージェントをディスパッチする。

**タスク:** fix-design.md の内容を既存設計書にマージし、バグ修正履歴（history.md）を初期作成する
...
**Output:** 既存設計書の更新、`{bugfix_dir}/history.md`

**ステータス分岐:**
- `DONE` → Step後処理を実行し、Step 15 へ
- `DONE_WITH_CONCERNS` → 懸念事項の内容をユーザーに報告し、対応方針を確認した上で Step 15 へ
- `NEEDS_CONTEXT` → 追加情報を提供して再ディスパッチ
- `BLOCKED` → ユーザーに報告し対応方針を確認

**Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step14`, step_title: `設計書反映`, artifact_dir: `{bugfix_dir}`）

#### Step 15: pending-issues 書き込み忘れチェック

**処理:**

1. `pending-issues-management (aide-powers skill: check)` を activate して実行する
   - 入力: progress_file_path: `{bugfix_dir}/bugfix-progress.md`, pending_issues_path: `.aide/specs/{feature_name}/pending-issues.md`
2. 共通スキルが進捗ファイルを遡り、書き込み漏れパターンを検索する
3. 戻り値による分岐:
   - **書き込み漏れなし** → Step 16 へ
   - **書き込み漏れあり** → ユーザーに確認の上で pending-issues.md に追記 → Step 16 へ
4. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step15`, step_title: `pending-issues 書き込み忘れチェック`, artifact_dir: `{bugfix_dir}`）
...
1. バグ修正内容サマリーを作成・提示する:
   - バグ報告（bug-report.md の概要）
   - 原因分析（bug-analysis.md の概要）
   - 修正方針（fix-plan.md の概要: 対策種別含む）
   - 修正内容（fix-design.md の概要）
   - 実装タスク（delta-task-list.md のタスク一覧）
2. 更新設計書一覧を提示する
3. テスト実行結果を提示する（全テスト・リグレッションテスト）
4. bugfix/ 配下のバグ修正履歴を提示する（bugfix_dir パス + 格納ドキュメント一覧）
...
6. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `step16`, step_title: `バグ修正完了の案内`, artifact_dir: `{bugfix_dir}`）
...
4. **次フェーズ遷移:** `fs-bugfix-phase3-final-check (aide-powers skill)` を activate して実行する

注: バグ修正ワークフローでは最終フェーズ（Phase 3 final-check）の進捗ファイル ✅ 完了 更新後に1回のみ git コミットを行う。本フェーズではコミットしない。

5. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-bugfix-phase2-impl`, step_id: `後処理`, step_title: `後処理`, artifact_dir: `{bugfix_dir}`）

## 完了条件

以下の全てを満たした状態:

1. fix-design.md が作成され、ユーザー承認 + QA APPROVED 済み（規模により分割構成の場合はメイン+全分割ファイルを含む）
2. delta-task-list.md が作成され、ユーザー承認済み
3. impl-process-checklist.md が作成され、全タスク・全ステップが完了済み
4. delta-task-list.md の全タスクが実装完了し、レビュー全PASS、テスト全PASS、リグレッションテスト全パス
5. ユーザーに動作検証を依頼済み
6. fix-design.md の内容が既存設計書にマージされ、history.md が初期作成されている
7. pending-issues 書き込み忘れチェックが完了している
8. バグ修正完了の案内がユーザーに提示されている
9. 進捗ファイル（bugfix-progress.md）の該当フェーズ行が `✅ 完了` に更新されている
...
- `multi-stage-code-review (aide-powers skill)` — Step 10 の各タスクで必ず実行
...
- `design-sync (aide-powers skill)` — Step 11（合理的乖離検出時）
- `doc-sync (aide-powers skill)` — Step 14（設計書反映）
- `pending-issues-management (aide-powers skill)` — Step 15 (check) / Step 16 (present)
...
- `bugfix-designer-prompt.md` — Step 2（mode: design / fix）
- `bugfix-task-planner-prompt.md` — Step 6
- `bugfix-doc-syncer-prompt.md` — Step 14
