...
## ルールファイルの配置
プラットフォームのルール注入機構（Claude Code の `.claude/rules/`、Kiro の `.kiro/steering/`、その他プラットフォーム固有の常時読み込み領域）に、`aide-powers-bootstrap`（「開発要求を受けたら using-aide-powers を activate せよ」というエントリポイント）、`aide-powers-global-rules`（全体共通ルール）、`aide-powers-phase-skill-rules`（フェーズスキル共通ルール）が常時適用ルールとして配置される。これらは rules-distribute が各プラットフォームの配置先へ配布・更新する。

## ツールマップ
skill はツール名を一般的な機能名（ファイル読み込み・ファイル作成・ファイル編集・コマンド実行・サブエージェント起動・スキル起動等）で記述している。実行するプラットフォーム（Claude Code / Kiro / Cursor / GitHub Copilot / Codex / Gemini CLI 等）ごとに実際に呼び出せるツール名は異なるため、`.aide/references/` 配下の該当プラットフォーム用ツールマップ（例: `kiro-ide-tools.md`, `codex-tools.md`, `vscode-copilot-tools.md` 等）を参照し、記述された機能名を自分が動作しているプラットフォームの実際のツール呼び出しに変換して実行すること。ツールマップを読まずに「そのツールは存在しない」と判断してはならない。使用可能なツール一覧は自分の実行環境から都度確認すること。
...
オーケストレータがサブエージェント起動ツールで呼び出す名前付きサブエージェント群。各フェーズスキル・共通スキルの定義に従い、指定された名前で直接呼び出す。汎用作業は aide-subagent、それ以外は専任エージェントを使う。
