...
## ツールマップ
スキルは Claude Code のツール名（Read / Write / Edit / Bash / Task / Skill）で記述されている。Kiro では `.aide/references/kiro-ide-tools.md`（IDE）または `kiro-cli-tools.md`（CLI）を参照してツール名を変換する（例: Read→read / Write→write / Edit→edit / Bash→shell / Task→subagent）。ツールマップを読まずに「そのツールは存在しない」と判断してはならない。
...
オーケストレータが `subagent` で呼び出す名前付きサブエージェント群。各フェーズスキル・共通スキルの定義に従い、指定された名前で直接呼び出す。汎用作業は aide-subagent、それ以外は専任エージェントを使う。
