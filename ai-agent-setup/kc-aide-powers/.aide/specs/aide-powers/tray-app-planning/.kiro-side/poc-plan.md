...
- **段階1: セッション開始時** — ハブスキル（using-aide）のみ注入。オーケストレーター選択ロジックと最小限のグローバルルールのみ
...
- **問題**: サブエージェントがusing-aide（ハブスキル）を読み込むと、オーケストレーター選択ロジックが不要にコンテキストを消費する
- **解決策**: using-aideスキルに`<SUBAGENT-STOP>`タグを配置し、サブエージェントとして起動された場合はハブスキルの読み込みを停止させる
- **適用箇所**: using-aide/SKILL.mdの冒頭付近に配置
