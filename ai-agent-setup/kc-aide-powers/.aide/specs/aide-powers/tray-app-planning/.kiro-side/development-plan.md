...
- using-aideメタスキル（オーケストレーター自動選択）を作成
...
| 成果物 | 内容 |
|---|---|
| CLAUDE.md | グローバルルール（~120行）: フェーズ省略禁止、敬語ルール、オーケストレーター実作業禁止、using-aideスキル呼び出し指示 |
| AGENTS.md | マルチプラットフォーム対応のエージェント設定 |
| using-aide/SKILL.md | メタスキル: 7オーケストレーターへの振り分けロジック |
...
| 3 | using-aideメタスキル | 7オーケストレーターの自動選択が正しく機能すること |
...
| REQ-M08: オーケストレーター自動選択 | using-aideメタスキルで正しく自動選択される |
...
| M2 | 基盤ファイル完成 | フェーズ2 | CLAUDE.md + using-aide + hooks + プラットフォーム設定が動作 |
