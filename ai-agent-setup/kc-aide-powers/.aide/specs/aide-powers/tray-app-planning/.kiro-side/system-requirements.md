...
│   ├── using-aide/SKILL.md        ← メタスキル（オーケストレーター自動選択）
...
| スクリプト | 言語 | 用途 |
|---|---|---|
| hooks/session-start | Shell（bash） | セッション開始時にusing-aide/SKILL.mdを読み込みJSON出力 |
...
| 状況 | 対応 |
|---|---|
| session-startスクリプト失敗 | セッションは継続。using-aideスキルの手動呼び出しで代替 |
...
| REQ-M08: オーケストレーター自動選択 | using-aideメタスキル + CLAUDE.md選択ガイド + descriptionベース自動選択 |
...
| スキル | 元ファイル | 行数見積 |
|---|---|---|
| using-aide（メタスキル） | orchestrator-index.md相当 | ~150行 |
