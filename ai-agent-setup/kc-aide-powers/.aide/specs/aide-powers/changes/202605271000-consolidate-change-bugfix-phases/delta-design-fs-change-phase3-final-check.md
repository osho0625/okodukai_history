# 新スキル詳細設計: fs-change-phase3-final-check

> このファイルは delta-design.md の付属設計書。新スキル `fs-change-phase3-final-check` の SKILL.md を実装するための完全仕様。

## スキルファイル配置

- パス: `skills/fs-change-phase3-final-check/SKILL.md`
- frontmatter:
...
# 最終整合性チェック（fs-change-phase3-final-check）
...
- **session-history-*.txt の確実な削除**: 検証完了後（PASS の場合）、`.aide/tmp/session-history-fs-change-phase*.txt` の全ファイルを必ず削除する。残存させると次回ワークフロー実行時に誤判定の原因となる
...
呼び出しパラメータ:
- skill_name: `fs-change-phase3-final-check`
...
1. **progress-resume-check (aide-powers skill)** を activate して実行する
   - 入力:
     - progress_file_path: `{changes_dir}/change-progress.md`
...
2. **phase-compliance-check (aide-powers skill: verify)** を activate して実行する — Phase 2 の署名を検証。FAIL の場合は Phase 2 に戻って後処理を再実行

3. **user-profile-management (aide-powers skill: apply)** を activate して実行する

4. `.aide/references/global-rules.md` を読み込み、内容に従う
5. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-change-phase3-final-check`, step_id: `前処理`, step_title: `前処理`）

### Step 1: セッションヒストリーの収集

**処理:**

1. `.aide/tmp/session-history-fs-change-phase1-analysis-*.txt` および `.aide/tmp/session-history-fs-change-phase2-impl-*.txt` の全ファイルパスを収集する
2. 全ファイルが存在することを確認する
3. 存在しないファイルがある場合 → ユーザーに報告し、該当フェーズの再実行を提案
4. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-change-phase3-final-check`, step_id: `step1`, step_title: `セッションヒストリーの収集`）
...
2. 渡す情報:
   ...
   - progress_file_path: `{changes_dir}/change-progress.md`
3. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-change-phase3-final-check`, step_id: `step2`, step_title: `検証用 agent の呼び出し`）
...
2. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-change-phase3-final-check`, step_id: `step3`, step_title: `検証結果の処理`）

### Step 4: 一時ファイルの削除

**処理:**

1. `.aide/tmp/session-history-fs-change-phase*.txt` の全ファイルを削除する
2. 削除完了を確認する
3. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-change-phase3-final-check`, step_id: `step4`, step_title: `一時ファイルの削除`）

### 後処理（フェーズ全体で1回のみ）

1. **doc-index-maintenance (aide-powers skill)** — ドキュメントインデックス更新
2. **user-profile-management (aide-powers skill: update)** — ユーザープロファイル更新
3. **git-commit-workflow (aide-powers skill)** — 変更ワークフロー全体の成果物をまとめてコミット（Docs: フッター付き）

注: phase-compliance-check (write) は実行しない。検証用 agent が自フェーズのステータス更新を実施するため。

4. **Step後処理:** step-history-writer (aide-powers skill) を activate して実行する（skill_name: `fs-change-phase3-final-check`, step_id: `後処理`, step_title: `後処理`）
...
- `git-commit-workflow (aide-powers skill)` — 後処理3（変更WF全体のコミット）
