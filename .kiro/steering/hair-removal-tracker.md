...
| RecordMode | 記録モード管理（複数ゾーン選択→一括記録） |
...
- 通常時: タップで情報表示（選択状態にしない）→ 情報パネル内「記録する」ボタンで記録モードに入り、そのゾーンを選択済みにする
- 記録モード: タップで選択トグル。記録モードバー（選択件数/まとめて記録/キャンセル）を表示。「まとめて記録」で選択ゾーンをバッチ保存モーダルへ（単一選択時は単一保存フロー）。保存後・マップ以外のタブへ移動時に自動解除
- スワイプ/ドラッグ選択は廃止（記録モードのタップ選択に統一）
- ページネーション: 履歴100件/ページ
