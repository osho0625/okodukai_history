...
├── manifest.json       # PWA設定
├── sw.js               # Service Worker (v340, Push通知対応)
...
- 未読入金: 🔔アイコン＋入金前残高表示
- 承認待ち: ✅アイコン（全ユーザーに表示）
- admin用: ⚙️（設定モーダル＝表示順変更）
- 🪴（ぷよ畑）、🕹️（ゲームセンター）、📋（お手伝いリスト）、📝（家族メモ帳、画面下部）、🔧（管理者認証）
...
- ⭐ お手伝いポイント表（20×20=400マス、複数枚対応、○枚目表示）
  ...
  - 400pt（1枚）コンプリート時にあそびチケット60分×2枚自動発行（かいせい/はるちか/いろは対象）
  - 枚コンプリート演出（フルスクリーン祝福＋紙吹雪＋チケットGET表示→次の枚スタート演出）
...
- 📈 残高グラフ（アコーディオン、transactionsから残高推移を逆算しCanvasで折れ線グラフ描画）
- 📋 履歴
...
- 📲 Push通知送信（admin/user/全員向けにカスタムPush通知送信）
...
| TSJ260826 | マージ済み | LaQ美術館追加 |
| feature/test | 作業中 | ポイント表枚コンプリート時チケット自動発行＆演出 |
| TSJ260831 | マージ済み | おうちブロードキャスト（Alexa連携 + ラズパイ読み上げ） |
