...
## 機能
1. 作品登録: 作者選択 → カメラ撮影（複数枚対応・×で削除可能）→ 作品名入力（任意）→ サムネイル選択 → 登録
2. ギャラリー: 作者ごとに作品数が多い順で表示、サムネイル＋題名の一覧
3. 作品詳細: 写真スワイプ閲覧
4. 管理者機能: 作品名変更、サムネイル変更、即時削除
5. ユーザー削除リクエスト: 子供が削除をリクエスト → TOPページでadminが承認/却下
...
|--------|------|------|
| id | UUID | PK |
| work_id | UUID | FK→laq_works.id (CASCADE) |
| requested_by | TEXT | リクエスト者名 |
