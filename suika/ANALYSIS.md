...
├── main.js                 ゲームメイン（全状態管理・セーブ/ロード・装備・勾玉・メニュー）
└── engine/
    ├── math.js             Vec3, Color, Mat4
    ├── renderer.js         ソフトウェア3Dレンダラ
    ├── input.js            キーボード/マウス/タッチ入力（isUp/isDown/isLeft/isRight+リピート）
    ├── touch-ui.js         スマホ用バーチャルパッド（スティック+A/B/◀▶/≡ボタン）
    ├── loader.js           バイナリアセット読込 + RLE展開
    ├── loop.js             固定タイムステップゲームループ（90ms）
    ├── stage.js            ステージデータパーサー
    ├── field.js            フィールド描画 + 移動 + 影 + NPC移動アニメ + パーティ追従
    ├── event.js            イベントスクリプト（全75コマンド完全対応）
    ├── message.js          メッセージ/選択肢UI（1文字/frame表示）
    ├── params.js           パラメータデータパーサー
    ├── battle.js           戦闘エンジン（スキル/アイテム/状態異常5種/クリティカル/AI5カテゴリ）
    ├── battle-ui.js        戦闘UI（プレイヤー3Dモデル+攻撃モーション+エフェクト）
    ├── audio.js            Web Audio SE再生（.auデコーダー）
    ├── shop.js             ショップUI（買う/売る）
    ├── quiz.js             クイズミニゲーム（32問）
    ├── compose.js          合成ショップ（14レシピ）
    ├── credits.js          スタッフロール
    └── password.js         復活の呪文デコーダー
pages/
├── suika.html              ゲームセンター版ページ（離脱確認+オートセーブ）
├── suika-original.html     原作Java版（CheerpJ 4.3）
└── arcade.html             ゲームセンター（カード追加済み）
```

### 勾玉システム（カスタム仕様）
- アイテムインデックス110〜126の17種類
- 各勾玉は7スキルを持ち、APを貯めて習得
- APは勾玉ごとに管理（外しても保持される — 原作は0リセット）
- 一度装備したキャラに紐付き（使い回し不可）
- 全7スキル習得で勾玉が砕け散る（消滅）
- 戦闘勝利時: AP = 敵パーティの合計AP × 10%
- GEM_DATA: 原作CGemData.javaから完全移植
- 装備制限: 120=主人公専用, 124=かるび専用, 125=うな専用, 他=フラグ条件

### 現在の残作業（優先度順）
1. 戦闘カメラ/視点の微調整（敵が大きすぎる問題が残る可能性）
2. 敵AI個別パターン（80+→現在5カテゴリ簡略化）
3. 個別スキルアニメーション（現在はパーティクル+スラッシュのみ）
4. パスワードセーブ生成（読込は可能だが生成は未実装）
5. 勾玉の実際の名前確認（param._daロード後に確認可能）
6. 戦闘中のプレイヤーモデル表示位置の微調整
...
1. **敵AI個別パターン拡充** — 5カテゴリ→20プロファイル（データ駆動方式）
   - 物理系5種（純物理/スキル付き/毒/デバフ/強攻撃）
   - 魔法系4種（単体/バランス/強力/全体）
   - サポート系3種（回復/バフ/蘇生）
   - 状態異常系3種（麻痺/混乱/ドレイン）
   - ボス系4種（サイクル/アグロ/サポート/最終）
   - 新アクション: 連続攻撃/HP吸収/蘇生/全体回復/バフ/デバフ
   - ターンサイクル制ボスAI（低HP時行動変化）
   - algo番号(1-107)→プロファイルマッピング完備
2. **個別スキルアニメーション** — 14種の視覚エフェクト実装
   - SLASH: 斬撃線 / FIRE: 炎バースト / ICE: 氷結晶
   - THUNDER: 雷撃 / WIND: 風螺旋 / HOLY: 聖光
   - DARK: 闇渦 / HEAL: 緑上昇光 / BUFF: 金オーラ
   - DEBUFF: 紫下降 / POISON: 緑泡 / DRAIN: 赤エネルギー流
   - MULTI_SLASH: 連続斬撃 / EXPLOSION: 爆発
   - ログメッセージ連動で自動トリガー
3. **パスワードセーブ生成** — 原作CPassCode/CPlayData完全準拠エンコーダー
   - 318文字パスワード生成（6行×53文字）
   - ヘッダー/キャラ3人/アイテム/フラグ/チェックサム
   - システムメニューに「じゅもん」追加
   - 全画面パスワード表示UI
4. **戦闘カメラ/モデル位置調整**
   - カメラ距離2100→2400、高さ1600→1700に調整
   - 注視点を(0,100,0)に上げてフレーミング改善
   - 敵モデルスケール0.9倍で大きすぎ問題解消
   - プレイヤーZ=-280、敵Z=+280に再配置
5. **キャラ名原作準拠確認**
   - 主人公: 西瓜太郎（デフォルト名、プレイヤー入力可）
   - 仲間1(index 1): うな
   - 仲間2(index 2): かるび
   - コメント修正（「もの専用」→「うな専用」）

### 次の作業予定
1. 勾玉の実際の名前確認（param._daロード後に表示確認）
2. 戦闘中プレイヤーモデル表示の微調整（実機確認後）
3. 仲間キャラ（うな/かるび）のパーティ加入イベント動作確認
4. ワールドマップ画面の改善
5. スタッフロール演出の改善
