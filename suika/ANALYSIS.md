...
1. **スキル/魔法システム実装** (battle.js)
   - INT基準の魔法ダメージ計算（原作MagicAttack準拠）
   - 回復魔法（原作Calc_IntHeal準拠）
   - バフ/デバフ/状態異常スキル
   - MP消費チェック
   - スキル対象タイプ（単体敵/全体敵/単体味方/全体味方/自分）
   - 敵AIの魔法攻撃対応

2. **アイテム使用実装** (battle.js)
   - 単体HP回復（原作CalcItemHeal準拠）
   - 全体HP回復
   - 毒治療
   - 蘇生（HP1/4回復）
   - アイテム消費処理

3. **戦闘後HP/MP反映** (main.js)
   - 戦闘終了時にplayerParamsへHP/MP同期
   - EXP/Gold獲得処理
   - フィールドに戻った後もダメージが持続

4. **戦闘UIスキル/アイテムメニュー** (battle-ui.js)
   - コマンドメニュー5項目化（こうげき/まほう/アイテム/ぼうぎょ/にげる）
   - スキル選択サブメニュー（MP表示、使用可否色分け）
   - アイテム選択サブメニュー
   - 味方ターゲット選択UI
   - スクロール対応（5件表示）

5. **毒ダメージ** — ターン終了時に毒状態のユニットにHP割合ダメージ

6. **状態異常フィールド追加** — stone, blind, confuse（将来拡張用）
