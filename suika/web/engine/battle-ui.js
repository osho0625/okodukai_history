// battle-ui.js 窶・Battle screen UI renderer with 3D model display
...
export class BattleUI {
  constructor(ctx, input, renderer, models, paramAll) {
    ...
    this.commands = [
      { label: '縺薙≧縺偵″', cmd: CMD.ATTACK },
      { label: '縺ｾ縺ｻ縺・, cmd: CMD.SKILL },
      { label: '繧｢繧､繝・Β', cmd: CMD.ITEM },
      { label: '縺ｼ縺・℃繧・, cmd: CMD.DEFEND },
      { label: '縺ｫ縺偵ｋ', cmd: CMD.RUN },
    ];
    ...
    if (this.phase === 'command') {
      ...
      if (this.input.isOK()) {
        ...
        if (skill.object === 0) { // single enemy
          ...
          // All targets or self 窶・execute immediately
          this.phase = 'command';
    ...
    for (let i = 0; i < totalEnemies; i++) {
      ...
      if (this.phase === 'target') {
        ...
        if (aliveIdx === this.targetCursor) {
          ...
          this.ctx.fillText('笆ｼ', screenPos.x, Math.min(screenPos.y - 30, 50));
        }
      }
    }
  }
  ...
  drawSkillMenu() {
    ...
    ctx.fillStyle = '#adf';
    ctx.textAlign = 'left';
    ctx.fillText('縺ｾ縺ｻ縺・(X:繧ゅ←繧・', x + 8, y + 12);
    ...
    for (let i = 0; i < maxShow && startIdx + i < this.playerSkills.length; i++) {
      ...
      const prefix = idx === this.subCursor ? '笆ｶ' : '  ';
      ctx.textAlign = 'left';
  ...
  drawItemMenu() {
    ...
    if (items.length === 0) {
      ...
      ctx.fillStyle = '#aaa';
      ctx.textAlign = 'left';
      ctx.fillText('繧｢繧､繝・Β縺後↑縺・, x + 8, y + 22);
      return;
    }
    ...
    ctx.fillStyle = '#afc';
    ctx.textAlign = 'left';
    ctx.fillText('繧｢繧､繝・Β (X:繧ゅ←繧・', x + 8, y + 12);
  ...
  drawAllyTargetSelect(battleState) {
    ...
    ctx.fillStyle = '#cfc';
    ctx.textAlign = 'left';
    ctx.fillText('縺繧後↓・・, x + 8, y + 12);

    for (let i = 0; i < players.length; i++) {
      ctx.fillStyle = i === this.targetCursor ? '#ff0' : (players[i].alive ? '#ddd' : '#666');
      const prefix = i === this.targetCursor ? '笆ｶ ' : '   ';
  ...
  drawTargetSelect(battleState) {
    ...
    ctx.fillStyle = '#fcc';
    ctx.textAlign = 'left';
    ctx.fillText('繧ｿ繝ｼ繧ｲ繝・ヨ:', x + 8, y + 14);
