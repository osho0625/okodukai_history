// compose.js 窶・Composition/crafting shop (ported from CComposition/CCompTable)
...
export class ComposeShop {
  ...
  open(playerGold, playerInventory) {
    ...
    this.message = '蜷域・螻九後＞繧峨▲縺励ｃ縺・∪縺帙・;
    this.messageTimer = 0;
  ...
  update() {
    ...
    if (this.input.isOK()) {
      ...
      if (this.canCraft(this.cursor)) {
        ...
        this.message = `${item ? item.name.trim() : '???'}繧貞粋謌舌＠縺滂ｼ～;
        this.messageTimer = 40;
      } else if (this.gold < r[1]) {
        this.message = '縺企≡縺瑚ｶｳ繧翫∪縺帙ｓ';
        this.messageTimer = 30;
      } else {
        this.message = '邏譚舌′雜ｳ繧翫∪縺帙ｓ';
        this.messageTimer = 30;
      }
    }
  ...
  draw() {
    ...
    ctx.fillText(this.message || '蜷域・螻・(X:髢峨§繧・', 200, 26);

    // Gold
    ctx.fillStyle = '#fd0';
    ctx.font = '10px sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText(`謇謖・≡: ${this.gold}G`, 385, 50);
    ...
    for (let i = 0; i < maxShow && startIdx + i < COMP_TABLE.length; i++) {
      ...
      const prefix = idx === this.cursor ? '笆ｶ' : '  ';
