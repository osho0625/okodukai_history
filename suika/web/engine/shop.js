// shop.js 窶・Shop system (ported from CToolShop/CInShop)
...
export class ShopUI {
  ...
  open(shopName, itemIndices, playerGold, playerInventory) {
    ...
    this.message = `${shopName}縲後＞繧峨▲縺励ｃ縺・∪縺帙港;
    this.messageTimer = 0;
    ...
    if (this.mode === 'main') {
      ...
      if (this.input.isOK()) {
        ...
        if (this.gold >= item.gold) {
          ...
          this.message = `${item.name}繧定ｲｷ縺｣縺滂ｼ～;
          this.messageTimer = 30;
        } else {
          this.message = '縺企≡縺瑚ｶｳ繧翫↑縺・ｼ・;
          this.messageTimer = 30;
        }
      }
      ...
      if (sellable.length === 0) { this.message = '螢ｲ繧後ｋ繧ゅ・縺後↑縺・; this.messageTimer = 30; this.mode = 'main'; this.cursor = 0; return; }
      ...
      if (this.input.isOK()) {
        ...
        this.message = `${s.name}繧・{s.gold}G縺ｧ螢ｲ縺｣縺歔;
        this.messageTimer = 30;
  ...
  draw() {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`謇謖・≡: ${this.gold} G`, 380, 58);
  ...
  _drawMainMenu(ctx) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      ctx.fillText((i === this.cursor ? '笆ｶ' : '  ') + items[i], x + 10, y + 22 + i * 24);
    }
  }
  ...
  _drawBuyMenu(ctx) {
    ...
    for (let i = 0; i < maxShow && startIdx + i < this.itemList.length; i++) {
      ...
      ctx.textAlign = 'left';
      ctx.fillText((idx === this.cursor ? '笆ｶ' : '  ') + item.name, x + 10, y + 18 + i * 22);
      ctx.textAlign = 'right';
