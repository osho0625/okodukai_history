// main.js 窶・Entry point for the HTML5 port of "縺吶＞縺九′鬟溘∋縺溘＞"
...
class SuikaGame {
  ...
  async init() {
    ...
    this.showLoadingScreen('蛻晄悄蛹紋ｸｭ...');

    try {
      this.showLoadingScreen('逕ｻ蜒剰ｪｭ霎ｼ荳ｭ...');
      this.images = await this.loader.loadAllImages(32);

      this.models = await this.loader.loadAllModels(204, (i, total) => {
        this.showLoadingScreen(`繝｢繝・Ν隱ｭ霎ｼ荳ｭ... ${i}/${total}`);
      });

      this.showLoadingScreen('繧ｹ繝・・繧ｸ繝・・繧ｿ隱ｭ霎ｼ荳ｭ...');
      ...
      this.showLoadingScreen('繧､繝吶Φ繝医ョ繝ｼ繧ｿ隱ｭ霎ｼ荳ｭ...');
      try {
      ...
      // Load parameter data
      this.showLoadingScreen('繝代Λ繝｡繝ｼ繧ｿ隱ｭ霎ｼ荳ｭ...');
      try {
      ...
      this.eventManager.itemCallback = (itemIdx, add) => {
        ...
        if (item && this.messageWindow) {
          this.messageWindow.show(`${item.name}繧呈焔縺ｫ蜈･繧後◆・～);
        }
      };
      ...
      this.eventManager.shopCallback = async (itemIndices, shopName) => {
        ...
        const result = await this.shopUI.open(shopName || '縺雁ｺ・, itemIndices, this.gold || 0, this.eventManager.inventory);
      ...
      this.eventManager.numberCallback = async (start) => {
        // Number selection 窶・use choice callback with numbers
      ...
      if (this.stageManager.stages.length > 0) {
        // Start at area 0, position (16, 35) 窶・matching original CInitGame
        let areaIdx = 0;
        const area = this.stageManager.stages[areaIdx];
      ...
      this.showLoadingScreen('隱ｭ縺ｿ霎ｼ縺ｿ螳御ｺ・ｼ・);
      this.state = 'title';
      // Default to "邯壹″縺九ｉ" if save data exists
      this.titleCursor = localStorage.getItem('suika_save') ? 1 : 0;
    } catch (e) {
      console.error('Load error:', e);
      this.showLoadingScreen('隱ｭ霎ｼ繧ｨ繝ｩ繝ｼ: ' + e.message);
      return;
    }
  ...
  updateTitle() {
    ...
    if (this.input.isOK()) {
      ...
      if (this.titleCursor === 0) {
        // New game 窶・play opening then start
        this.resetNewGame();
        this.state = 'opening';
        this.openingFrame = 0;
        ...
        // Password input
        const pw = prompt('蠕ｩ豢ｻ縺ｮ蜻ｪ譁・ｒ蜈･蜉帙＠縺ｦ縺上□縺輔＞:');
        if (pw) {
  ...
  drawTitle() {
    ...
    ctx.fillText('縺吶＞縺九′鬟溘∋縺溘＞', 200, 80);
    ...
    // Menu
    const menuY = 170;
    const items = ['蛻昴ａ縺九ｉ', '邯壹″縺九ｉ', '蠕ｩ豢ｻ縺ｮ蜻ｪ譁・];
    ctx.font = '16px sans-serif';
    for (let i = 0; i < items.length; i++) {
      ctx.fillStyle = i === this.titleCursor ? '#ff0' : '#fff';
      const prefix = i === this.titleCursor ? '笆ｶ ' : '   ';
    ...
    if (localStorage.getItem('suika_save')) {
      ...
      ctx.fillStyle = '#8f8';
      ctx.fillText('繧ｻ繝ｼ繝悶ョ繝ｼ繧ｿ縺ゅｊ', 200, 258);
    }

    // Credits
    ctx.font = '11px sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.6)';
    ctx.fillText('2002-2008 陬ｽ菴懊・闡嶺ｽ・縺上ｍ縺吶￠', 200, 295);

    // Touch hint (only on touch devices)
    ctx.font = '10px sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.fillText('繧ｹ繝・ぅ繝・け竊鯛・縺ｧ驕ｸ謚・/ A縺ｧ豎ｺ螳・, 200, 312);
  }

  // --- Opening sequence (matching original CTitle.Opening) ---
  updateOpening() {
    this.openingFrame++;
    // Total duration: ~180 frames (4 size stages ﾃ・36 brightness cycles + fade)
    // Skip on OK press
  ...
  drawOpening() {
    ...
    else fontSize = 80 + stageProgress * 60; // 80竊・40px, overflows screen

    // Brightness pulse (0竊・55竊・ per stage)
    let brightness;
    if (stage < 4) {
    ...
    ctx.fillText('縺吶＞縺九′鬟溘∋縺溘＞', 200, 160);
    ...
    if (f > 40 && f < 190) {
      ...
      ctx.font = '10px sans-serif';
      ctx.fillText('Enter / A 縺ｧ繧ｹ繧ｭ繝・・', 200, 305);
    }
  }
  ...
  drawGame() {
    ...
    if (this.battleResult) {
      ...
      for (const lu of this.battleResult.levelUps) {
        ...
        this.ctx.fillText(`${lu.name} Lv${lu.prevLv}竊・{lu.newLv}!`, 200, y);
        y += 20;
      }
  ...
  applyExp(player, exp) {
    ...
    if (player.lv > prevLv) {
      console.log(`${player.name} Lv${prevLv}竊・{player.lv}!`);
    }
  }

  // --- System Menu ---
  updateMenu() {
    const items = ['繧ｹ繝・・繧ｿ繧ｹ', '陬・ｙ', '繝槭ャ繝・, '繧ｻ繝ｼ繝・, '繧ｿ繧､繝医Ν縺ｸ', '髢峨§繧・];
  ...
  updateEquipMenu() {
    ...
    if (eq.phase === 'chr') {
      ...
      const slots = ['豁ｦ蝎ｨ', '髦ｲ蜈ｷ', '逶ｾ', '陬・｣ｾ1', '陬・｣ｾ2'];
      ...
      equippable.unshift({ invIdx: -1, item: { name: '縺ｯ縺壹☆' }, idx: -1 }); // unequip option
  ...
  drawMenu() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const prefix = i === this.menuCursor ? '笆ｶ' : '  ';
    ...
    for (let i = 0; i < this.playerParams.length; i++) {
      ...
      // Show equipment
      if (p.equip) {
        const slotNames = ['豁ｦ', '髦ｲ', '逶ｾ', '鬟ｾ', '鬟ｾ'];
        let eqStr = '';
        for (let s = 0; s < 5; s++) {
          if (p.equip[s] >= 0) {
    ...
    ctx.fillStyle = '#fd0';
    ctx.fillText(`謇謖・≡: ${this.gold || 0} G`, sx + 10, py + 5);
  }
  ...
  drawEquipMenu(ctx) {
    ...
    if (eq.phase === 'chr') {
      ctx.fillStyle = '#adf';
      ctx.fillText('陬・ｙ縺吶ｋ繧ｭ繝｣繝ｩ繧帝∈謚・', sx + 10, sy + 20);
      for (let i = 0; i < this.playerParams.length; i++) {
        ctx.fillStyle = i === eq.chrIdx ? '#ff0' : '#fff';
        ctx.fillText((i === eq.chrIdx ? '笆ｶ' : '  ') + this.playerParams[i].name, sx + 10, sy + 42 + i * 22);
      }
    } else if (eq.phase === 'slot') {
      const p = this.playerParams[eq.chrIdx];
      ctx.fillStyle = '#ff0';
      ctx.fillText(`${p.name} 縺ｮ陬・ｙ:`, sx + 10, sy + 20);
      const slots = ['豁ｦ蝎ｨ', '髦ｲ蜈ｷ', '逶ｾ', '陬・｣ｾ1', '陬・｣ｾ2'];
      for (let i = 0; i < slots.length; i++) {
        ctx.fillStyle = i === eq.slot ? '#ff0' : '#fff';
        const equipped = (p.equip && p.equip[i] >= 0) ? this.paramAll.getItem(p.equip[i]) : null;
        const eqName = equipped ? equipped.name.trim() : '縺ｪ縺・;
        ctx.fillText((i === eq.slot ? '笆ｶ' : '  ') + `${slots[i]}: ${eqName}`, sx + 10, sy + 42 + i * 22);
      }
    } else if (eq.phase === 'item') {
      ...
      const equippable = this.eventManager.inventory
        .map((idx, i) => ({ invIdx: i, item: this.paramAll.getItem(idx), idx }))
        .filter(e => e.item && e.item.kind === slotKind);
      equippable.unshift({ invIdx: -1, item: { name: '縺ｯ縺壹☆' }, idx: -1 });

      ctx.fillStyle = '#adf';
      ctx.fillText('陬・ｙ縺吶ｋ繧｢繧､繝・Β:', sx + 10, sy + 20);
      const maxShow = 10;
      ...
      for (let i = 0; i < maxShow && startIdx + i < equippable.length; i++) {
        ...
        ctx.fillText((idx === eq.itemCursor ? '笆ｶ' : '  ') + (e.item.name || '').trim(), sx + 10, sy + 42 + i * 20);
      }
    }
  }
  ...
  drawGameOver() {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('・ｧ・｡・ｭ・･縲・ｯ・ｶ・･・ｲ', 200, 160);
  ...
  drawWorldMap() {
    ...
    ctx.font = '11px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('繝ｯ繝ｼ繝ｫ繝峨・繝・・ (Enter/X縺ｧ髢峨§繧・', 200, 11);
  }
  ...
  saveGame(silent = false) {
    ...
    if (!silent && this.messageWindow) this.messageWindow.show('繧ｻ繝ｼ繝悶＠縺ｾ縺励◆');
  }
